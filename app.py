...
from tools import read_timesheet_data, create_invoice_document, save_or_update_timesheet, send_message_with_attachments, calculate_salary
...
1. Use `read_timesheet_data` to get data for the current or user-specified month.
   - The filename is in the format of "timesheet_<monthName>.xlsx"
...
Note: To calculate the salary use the tool `calculate_salary` which needs the present days and pay per day as parameters
4. Build the invoice dictionary in the following format:
    {{
        ...
        "total": "<salary>/-",
        "total_words": "Rs. <salary in words> Only"
    }}
